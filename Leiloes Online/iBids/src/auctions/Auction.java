...
public class Auction {
    ...
    public String getAuctionData () {
        if (results == null) return String.format("%s:::%s:::%s:::%s:::%s:::%s", ((this.active) ? "1" : "0"),Crypto.getStringFromKey(publicKey),
                                Crypto.getStringFromKey(seller), description, Double.toString(basePrice), Long.toString(this.getTimeLeft()));
        else return String.format("%s:::%s:::%s:::%s:::%s:::%s:::%s", ((this.active) ? "1" : "0"), Crypto.getStringFromKey(publicKey),
    ...
    public SealedObject getBid (String user) {
        if (!offers.containsKey(Crypto.hash(user,"SHA-256"))) return null;
        return offers.get(user);
    }
